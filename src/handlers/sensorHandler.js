...
export class SensorHandler {
  static async read() {
    try {
      const readFromSensorPromises = _.map(config.sensors, sensor => sensor.read().reflect());
      const measurements = [];
      await Promise.all(readFromSensorPromises).each((inspection, index) => {
        ...
        if (inspection.isFulfilled()) {
          ...
          if (!_.isUndefined(temperatureValue)) {
            const temperatureMeasurement = new TemperatureMeasurement(
              temperatureValue,
              sensor.temperatureUnits,
              sensor.measurementLocation,
            );
            Log.logInfo(`${temperatureMeasurement}`);
            measurements.push(temperatureMeasurement);
          }
          if (!_.isUndefined(humidityValue)) {
            const humidityMeasurement = new HumidityMeasurement(
              humidityValue,
              sensor.humidityUnits,
              sensor.measurementLocation,
            );
